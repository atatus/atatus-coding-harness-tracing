...
class TestHooksJson:
    ...
    def test_no_forbidden_tokens_in_commands(self, data):
        for event, cmd in self._iter_commands(data):
            ...
            assert "~/.atatus" not in cmd, f"{event}: command contains '~/.atatus': {cmd}"
...
class TestPyproject:
    ...
    def test_entry_point_for_cursor(self, parsed):
        ...
        assert scripts["atatus-hook-cursor"] == "tracing.cursor.hooks.handlers:main"
...
class TestRunHook:
    ...
    def test_references_cursor_entry_point(self):
        assert "atatus-hook-cursor" in RUN_HOOK.read_text()
