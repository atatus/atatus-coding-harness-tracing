...
PLUGIN = Path(__file__).parents[3] / "tracing" / "opencode" / "plugin" / "atatus-tracing.ts"
...
def _instrumented_plugin(tmp_path: Path) -> Path:
    ...
    source = source.replace(
        marker,
        marker + "  globalThis.__atatusPayloads.push(payload)\n  return\n",
    )
...
def test_foreign_root_messages_are_not_forwarded(tmp_path: Path) -> None:
    ...
    result = _run_node(
        ...
        const {{ AtatusTracing }} = await import({json.dumps(plugin.as_uri())});
        const messages = [{{
        ...
        const hooks = await AtatusTracing({{
          client: {{ session: {{
...
def test_sdk_error_result_does_not_fetch_or_forward_root_transcript() -> None:
    result = _run_node(
        f"""
        import {{ AtatusTracing }} from {json.dumps(PLUGIN.as_uri())};
        let messagesCalls = 0;
        const hooks = await AtatusTracing({{
          client: {{ session: {{
