...
def cwd_tmp(tmp_path, monkeypatch):
    ...
    def _fake_venv_bin(name):
        return tmp_path / ".atatus" / "harness" / "venv" / "bin" / name
...
class TestInstallFreshStructure:
    ...
    def test_top_level_contains_atatus_tracing(self, settings_file, monkeypatch):
        _mock_prompts(monkeypatch)
        install()
        data = json.loads(settings_file.read_text())
        assert _ac.HOOK_NAME in data
        assert _ac.HOOK_NAME == "atatus-tracing"

    def test_atatus_tracing_has_both_events(self, settings_file, monkeypatch):
        _mock_prompts(monkeypatch)
        install()
        data = json.loads(settings_file.read_text())
...
class TestIdempotent:
    """Running install twice yields a single atatus-tracing block."""
    ...
    def test_no_duplicate_block(self, settings_file, monkeypatch):
        ...
        install()
        data = json.loads(settings_file.read_text())
        # Only one atatus-tracing block, with each event present exactly once
        assert _ac.HOOK_NAME in data
        for event in _ac.EVENTS:
            handlers = data[_ac.HOOK_NAME][event]
            assert len(handlers) == 1
...
class TestUninstall:
    def test_uninstall_removes_only_atatus_tracing(self, settings_file, monkeypatch):
        settings_file.parent.mkdir(parents=True, exist_ok=True)
