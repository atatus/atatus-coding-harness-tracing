...
def _extract_turn_from_rollout(rollout_path: Path, turn_id: str) -> "dict | None":
    ...
    try:
        with open(rollout_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                # session_meta is the rollout's first record and carries the authoritative
                # provider id for the whole session. It arrives before any task_started, so
                # it must be read outside the in_turn gate below.
    ...
    if observed_fields:
        ...
        # Codex derives non_cached_input_tokens rather than serializing it; we do the same,
        # but only when both input and cached are observed, else we'd fabricate a number
        # from unknown cache data.
...
def _llm_identity(model: "object", provider: "object") -> "tuple[str, str | None]":
    ...
    ``llm.system`` is the AI product/model family; ``llm.provider`` is the hosting
    provider -- kept distinct because e.g. DeepSeek models can be hosted on Azure.

    Per OpenInference, a well-known ``llm.system`` value MUST be used when one applies:
    a model-family match on the model name, else the provider when it is ``openai``/
    ``azure``, else ``"openai"`` when the provider is absent (session_meta's default),
    else the provider id itself as a spec-permitted custom value.
    """
