...
## Setup
The installer prompts for your Atatus license key and project name, writes credentials to `~/.atatus/harness/config.json`, and registers Copilot Chat hooks at `.github/hooks/hooks.json`.
...
```bash
curl -sSL https://raw.githubusercontent.com/atatus/coding-harness-tracing/main/install.sh | bash -s -- copilot
```

Uninstall:

```bash
curl -sSL https://raw.githubusercontent.com/atatus/coding-harness-tracing/main/install.sh | bash -s -- uninstall copilot
```
